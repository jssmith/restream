  ...
  override def messageReceived(ctx: ChannelHandlerContext, msg: MessageEvent): Unit = {
    ...
    msg.getMessage.asInstanceOf[Command] match {
      case c : InitReplayCommand[_] => {
        ...
        server.startLatch = new CountDownLatch(runConfig.numPhases * runConfig.numPartitions)
              ...
              new Thread(s"replay-$workerId-$phaseId") {
                ...
                override def run(): Unit = {
                  ...
                  try {
                    ...
                    logger.info(s"starting replay on partition $workerId (phase $phaseId)")
                    ...
                    readerThread.readEvents(e => {
                      if (e.ts >= batchEndTimestamp) {
                        logger.info(s"reached batch end on partition $workerId phase $phaseId")
                        ...
                        logger.info(s"advancing endtime $batchEndTimestamp on partition $workerId phase $phaseId")
                    ...
                    server.stateCommunicationService.finalizeBatch(phaseId, batchEndTimestamp)
                    logger.info(s"finished phase $phaseId on partition $workerId")
