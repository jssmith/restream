...
import replaydb.runtimedev.distributedImpl.ReplayMapTopKLazyImpl.{StateResponseTopK, StateReadTopK}
...
import replaydb.runtimedev.{ReplayMap, BatchInfo, ReplayMapTopK}

import scala.reflect.ClassTag
import scala.collection.JavaConversions._

class ReplayMapTopKLazyImpl[K, V](default: V, collectionId: Int,
  ...
  val internalReplayMapTopK: ReplayMapTopK[K, V] with Threaded =
    new replaydb.runtimedev.threadedImpl.ReplayMapTopKLazyImpl[K, V](default)
  ...
  override def getPrepareTopK(ts: Long, count: Int)(implicit batchInfo: BatchInfo): Unit = {
    for (i <- 0 until commService.numWorkers) {
      queuedLocalReadPrepares(batchInfo.phaseId, i, batchInfo.batchEndTs).offer(StateReadTopK(ts, count))
    }
  }
  ...
  def requestRemoteTopKRead(ts: Long, k: Int): TopK = {
    internalReplayMapTopK.getTopK(ts, k)
  }
