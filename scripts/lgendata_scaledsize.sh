...
if [[ $# -lt 4 ]]; then
  echo "Usage: ./lgendata_scaledsize.sh size_spec num_events num_users split_nums [ keep_only=-1 ] [ partitioned=false ]"
  echo "e.g.: ./lgendata_scaledsize.sh 5m-each 5000000 100000 \"1 2 4 8 16 32\" 1 true"
...
for i in ${SPLIT_NUMS[*]}; do
  ...
  java -cp /home/ec2-user/replaydb-worker/replaydb-apps-assembly-0.1-SNAPSHOT.jar replaydb.exec.EventGenerator tunable $NUM_USERS $NUM_EVENTS $ROOT_DIR/events-$SIZE_SPEC-split-$i/events.out $i $KEEP_ONLY $PARTITIONED
done
