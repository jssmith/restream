...
class Utils(profileName: String = "default", region: String = "us-west-2") {

  def regionUrl = s"ec2.$region.amazonaws.com"
  ...
  def imageId = {
    region match {
      ...
      case _ => throw new RuntimeException(s"no image known for region $region")
    }
  }

  def getEC2ClientAndCredentials: (AmazonEC2Client, ProfileCredentialsProvider) = {
    val credentials = new ProfileCredentialsProvider(profileName)
    val ec2client = new AmazonEC2Client(credentials)
    ec2client.setEndpoint(regionUrl)
