...
//  }
  ...
  for (i <- filenames.indices) {
    clients.issueCommand(i, new InitReplayCommand(partitionId = i, filename = filenames(i),
      classOf[SpamDetectorStats], runConfiguration, printProgressInterval = 100000))
  }
